...
import com.bonlimousin.gateway.config.KafkaProperties;
...
class BonGatewayKafkaResourceIT {
    ...
    void setup() {
        ...
        BonGatewayKafkaResource kafkaResource = new BonGatewayKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/bon-gateway-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/bon-gateway-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
